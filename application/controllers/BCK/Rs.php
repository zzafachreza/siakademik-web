...
class Rs extends CI_Controller{
	...
	function insert(){
           ...
           $myPdf = $this->uploadFoto('filepdf_'.$this->modul(),date('ymdhis'));
          ...
          $no2=1;
           ...
           foreach($this->db->query("SHOW COLUMNS from `data_".$this->modul()."` WHERE FIELD !='id_".$this->modul()."'")->result() as $col){
        	    ...
        	    if($data_field=="file_".$this->modul()){
        	        $data_field = $myFoto;
        	    }elseif($data_field=="filepdf_".$this->modul()){
        	        $data_field = $myPdf;
        	    }else{
...
$objWriter->save('php://output');
	...
	function delete(){
		...
		$sqlFoto = "SELECT file_".$this->modul()." FROM data_".$this->modul()." WHERE id_".$this->modul()."='$id'";
	...
	function update(){
	      ...
	      $pdf_old = $_POST['filepdf_'.$this->modul().'_old'];
      ...
      if(!empty($_FILES['file_'.$this->modul()]['name'])){
       ...
       if(!empty($_FILES['filepdf_'.$this->modul()]['name'])){
