  	...
  	<button class="btn bg-utama" type="SUBMIT"><i class="flaticon2-files-and-folders"></i> Simpan</button>
    	...
    	<?php 
        			...
        			<?php echo ucfirst(str_replace("_"," ",$col->Field)) ?> 
        			  ...
        			  <?php 
    	  			    }elseif($col->Field=="fid_materi" || $col->Field=="kode"){
    	  			        ?>
        			  ...
        			  <?php 
        			   ...
        			   <select  name="<?php echo $col->Field ?>" class="form-control selectza">
        			       ...
        			       foreach($this->db->query("SELECT * FROM data_user")->result() as $r){
        			           
        			       
        			       ?>
        			       <option <?php echo $data->{$col->Field}==$r->id?'selected':''?> value="<?php echo $r->id ?>"><?php echo $r->nama_lengkap ?></option>
        			       
        			       <?php } ?>
        			      
        			        <option =='PDF'?'selected':'' ?> value="PDF">PDF</option>
